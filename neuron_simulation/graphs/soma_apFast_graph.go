...
// This graph renders a soma's apFast values

type somaAPFastGraph struct {
...
// NewSomaAPFastGraph creates imgui graph
func NewSomaAPFastGraph() api.IGraph {
	o := new(somaAPFastGraph)
...
func (g *somaAPFastGraph) Draw(environment api.IEnvironment, vertPos int) {
...
func (g *somaAPFastGraph) drawHeader(environment api.IEnvironment) {
	if imgui.TreeNode("Controls##6") {
		config := environment.Config()
		...
		changedS := imgui.DragIntV("RangeStart##6", &rangeStart, 1.0, 0, int32(moData.RangeEnd), "%d")

		imgui.SameLine()

		changedE := imgui.DragIntV("RangeEnd##6", &rangeEnd, 1.0, rangeStart, duration, "%d")
...
func (g *somaAPFastGraph) drawGraph(environment api.IEnvironment) {
...
func (g *somaAPFastGraph) drawVerticalMarkers(config api.IModel, drawList imgui.DrawList) {
	// Mapped data coords
	uX := 0.0
	wX := 0.0
...
func (g *somaAPFastGraph) drawData(environment api.IEnvironment, drawList imgui.DrawList) {
	config := environment.Config()
	moData, _ := config.Data().(*model.ConfigJSON)
	rangeStart := int32(moData.RangeStart)
	rangeEnd := int32(moData.RangeEnd)
