...
// This graph renders a synapse's PSP values

type synapsePspGraph struct {
...
// NewSynapsePspGraph creates imgui graph
func NewSynapsePspGraph() api.IGraph {
	o := new(synapsePspGraph)
...
func (g *synapsePspGraph) Draw(environment api.IEnvironment, vertPos int) {
...
func (g *synapsePspGraph) drawHeader(environment api.IEnvironment) {
	if imgui.TreeNode("Controls##3") {
		config := environment.Config()
...
func (g *synapsePspGraph) drawGraph(environment api.IEnvironment) {
...
func (g *synapsePspGraph) drawVerticalMarkers(config api.IModel, drawList imgui.DrawList) {
	// Mapped data coords
	uX := 0.0
	wX := 0.0
...
func (g *synapsePspGraph) drawData(environment api.IEnvironment, drawList imgui.DrawList) {
	config := environment.Config()
	...
	for t := rangeStart; t < rangeEnd; t++ {
		if len(activeSamples) > 0 {
			...
			uY := MapSampleToUnit(sY, samples.SynapsePspMin(), samples.SynapsePspMax())
