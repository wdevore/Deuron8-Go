...
// This graph renders a synapse's surge values

type synapseSurgeGraph struct {
...
// NewSynapseSurgeGraph creates imgui graph
func NewSynapseSurgeGraph() api.IGraph {
	o := new(synapseSurgeGraph)
...
func (g *synapseSurgeGraph) Draw(environment api.IEnvironment, vertPos int) {
...
func (g *synapseSurgeGraph) drawHeader(environment api.IEnvironment) {
	if imgui.TreeNode("Controls##2") {
		config := environment.Config()
		...
		changedS := imgui.DragIntV("RangeStart##2", &rangeStart, 1.0, 0, int32(moData.RangeEnd), "%d")

		imgui.SameLine()

		changedE := imgui.DragIntV("RangeEnd##2", &rangeEnd, 1.0, rangeStart, duration, "%d")
...
func (g *synapseSurgeGraph) drawGraph(environment api.IEnvironment) {
...
func (g *synapseSurgeGraph) drawVerticalMarkers(config api.IModel, drawList imgui.DrawList) {
	// Mapped data coords
	uX := 0.0
	wX := 0.0
...
func (g *synapseSurgeGraph) drawData(environment api.IEnvironment, drawList imgui.DrawList) {
	config := environment.Config()
	...
	for t := rangeStart; t < rangeEnd; t++ {
		if len(activeSamples) > 0 {
			...
			uY := MapSampleToUnit(sY, samples.SynapseSurgeMin(), samples.SynapseSurgeMax())
